...
import { getRenamedreturns } from 'apiSdk/renamedreturns';
...
function DashboardPage() {
  ...
  const renamedreturnStat = useSWR<PaginatedInterface<RenamedreturnInterface>>(
    () => `/renamedreturns`,
    () => getRenamedreturns({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Renamedreturns',
      result: renamedreturnStat?.data?.totalCount,
      isLoading: renamedreturnStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Accounts',
